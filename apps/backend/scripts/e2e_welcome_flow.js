(async function () {
  ...
  if (!createRes.ok) {
    console.error('Create automation failed', createJson);
    process.exit(1);
  }
  const automationId =
    createJson.id ||
    (createJson.data && createJson.data.id) ||
    createJson.automation?.id;
  ...
  if (!contactRes.ok) {
    console.error('Create contact failed', contactJson);
    process.exit(1);
  }
  ...
  while (Date.now() < timeout) {
    ...
    const res = await fetch(`${BASE}/automations/${automationId}`, {
      headers: { Authorization: `Bearer ${token}` },
    });
    const j = await res.json();
    const sendCount =
      j.sendCount ?? j.send_count ?? (j._count && j._count.executions) ?? 0;
    console.log('sendCount=', sendCount);
    if (sendCount > 0) {
      console.log(
        'SUCCESS: welcome automation executed, sendCount=',
        sendCount,
      );
      process.exit(0);
    }
