(async () => {
  try {
    ...
    const ensureTenantSegment = async (tenantAccountId) => {
      let contact = await prisma.contact.findFirst({
        where: {
          ...
          OR: [{ email: accountEmail }, { phone: phone }],
        },
      });
      if (!contact) {
      ...
      let segment = await prisma.segment.findFirst({
        where: { accountId: tenantAccountId, name: 'Smoke Test Segment' },
      });
      if (!segment) {
        ...
        blocks: [
          {
            ...
            type: 'text',
            content: {
              text: "Hello Romuald — c'est un test de smoke.",
              fontSize: 14,
            },
          },
        ],
    ...
    const emailId =
      createdEmail.id || (createdEmail.data && createdEmail.data.id);
    if (!emailId) {
    ...
    if (createdEmail.accountId) {
      const tenantAccount = await prisma.account.findUnique({
        where: { id: createdEmail.accountId },
      });
      const tenantEmail =
        (tenantAccount && tenantAccount.adminEmail) || account.adminEmail;
      token = jwt.sign(
