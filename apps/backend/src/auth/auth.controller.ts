...
export class AuthController {
  ...
  @ApiParam({ name: 'token', description: 'Token de confirmation reçu par email' })
  ...
  async markOnboardingCompleted(@Request() req) {
    // req.user contient accountId grâce au JwtStrategy
    return this.authService.markOnboardingCompleted(req.user.accountId);
  }
  ...
  async generateTwoFactorSecret(@Request() req) {
    return this.authService.generateTwoFactorSecret(req.user.accountId);
  }
  ...
  async enableTwoFactor(@Request() req, @Body() body: { code: string }) {
    return this.authService.enableTwoFactor(req.user.accountId, body.code);
  }
  ...
  async disableTwoFactor(@Request() req) {
    return this.authService.disableTwoFactor(req.user.accountId);
  }

  @Post('send-2fa-sms')
  @UseGuards(JwtAuthGuard)
  @ApiOperation({ summary: 'Envoyer un code 2FA par SMS (placeholder)' })
  async sendTwoFactorSms(@Request() req, @Body() body: { phone?: string }) {
    return this.authService.sendTwoFactorSms(req.user.accountId, body.phone);
  }
  ...
  async me(@Request() req) {
    return this.authService.getAccount(req.user.accountId);
  }
}