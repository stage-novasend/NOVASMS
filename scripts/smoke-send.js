(async () => {
  try {
    ...
    let contact = await prisma.contact.findFirst({
      where: { accountId: account.id, OR: [{ email: accountEmail }, { phone: phone }] },
    });
    if (!contact) {
    ...
    let segment = await prisma.segment.findFirst({
      where: { accountId: account.id, name: 'Smoke Test Segment' },
    });
    if (!segment) {
      segment = await prisma.segment.create({
        data: { accountId: account.id, name: 'Smoke Test Segment', type: 'manual', criteria: {} },
      });
    ...
    const token = jwt.sign({ sub: account.id, email: account.adminEmail }, secret, {
      expiresIn: '8h',
    });
        ...
        blocks: [
          {
            id: 'blk-1',
            type: 'text',
            content: { text: "Hello Romuald — c'est un test de smoke.", fontSize: 14 },
          },
        ],
      },
    };

    let res = await fetch(apiBase + '/campaigns', {
      method: 'POST',
    ...
    if (!emailId) {
      console.error('Failed to create email campaign');
      process.exit(1);
    }

    res = await fetch(apiBase + `/campaigns/${emailId}/send`, {
      method: 'POST',
    ...
    res = await fetch(apiBase + '/campaigns', {
      method: 'POST',
    ...
    if (!smsId) {
      console.error('Failed to create sms campaign');
      process.exit(1);
    }

    res = await fetch(apiBase + `/campaigns/${smsId}/send`, {
      method: 'POST',
      body: JSON.stringify({ immediateOrScheduled: 'immediate' }),
      headers: { 'Content-Type': 'application/json', Authorization: 'Bearer ' + token },
    });
